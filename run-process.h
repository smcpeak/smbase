...
private:     // data
  // Command to run.
  std::vector<OldSmbaseString> m_command;
...
public:      // methods
  ...
  // strings).
  void setCommand(std::vector<OldSmbaseString> const &command);
  ...
  //   - "Aborted"
  //   - "Signal N"
  OldSmbaseString exitDescription() const;

  // Run 'command'.  Throw if not 'exitedWith0()'.
  static void check_run(std::vector<OldSmbaseString> const &command);
  ...
  static void buildWindowsCommandLine(std::vector<char> &commandLine,
    std::vector<OldSmbaseString> const &command);
};
