...
void testMatchResultsIterable()
{
...

  {
    int matchNum = 0;
    for (MatchResults m :
           MatchResultsIterable("xabcddey abcddde abcdddde", re)) {
      switch (matchNum++) {
        case 0:
          EXPECT_EQ(m.asVector(),
                    (std::vector<std::string>{"abcdde", "b", "dd"}));
          break;
  ...
  {
    int matchNum = 0;
    for (MatchResults m : MatchResultsIterable("abc", re)) {
      ++matchNum;
    }
    xassert(matchNum == 0);
  }
